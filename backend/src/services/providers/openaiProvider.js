/**
 * OpenAI Provider for AI Intelligence Layer
 * Sends structured prompt to OpenAI Chat Completions API
 */
...
const generateInsights = async (lead) => {
  ...
  const systemPrompt = `You are a professional real estate CRM AI Assistant.
...
Return ONLY a valid raw JSON object matching this schema. Do not enclose in markdown blocks, do not write explanations.`;
  ...
  const userPrompt = `Here is the lead's current profile data:
- Name: ${lead.full_name}
...
- Expected Revenue: ₹${lead.expected_revenue || 0}`;
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'gpt-4o-mini', // cost-effective standard model
      messages: [
